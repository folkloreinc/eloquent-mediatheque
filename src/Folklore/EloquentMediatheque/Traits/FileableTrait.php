...
trait FileableTrait
{
    ...
    {
        return config(
            'mediatheque.fileable.destination',
            '{mediatheque_type}/{date(Y-m-d)}/{id}-{date(his)}.{extension}'
        );
    }
    ...
    public function setFile($path, $file = array())
    {
        ...
        $extension = pathinfo(
            array_get($file, 'original', $path),
            PATHINFO_EXTENSION
        );

        //Get original file name
        $deleteOriginalFile = config(
            'mediatheque.fileable.delete_original_file',
            false
        );
        ...
        $defaultFile['extension'] = isset($extensions[$defaultFile['mime']])
            ? $extensions[$defaultFile['mime']]
            : $extension;
        ...
        $file['filename'] = $this->parseFileableDestination(
            $destination,
            $replaces
        );
        ...
        if ($this instanceof SizeableInterface) {
            $modelData[$this->getWidthColumnName()] = array_get(
                $file,
                'width',
                0
            );
            $modelData[$this->getHeightColumnName()] = array_get(
                $file,
                'height',
                0
            );
        ...
        if ($this instanceof TimeableInterface) {
            $modelData[$this->getDurationColumnName()] = array_get(
                $file,
                'duration',
                0
            );
        ...
        if ($this instanceof PaginableInterface) {
            $modelData[$this->getPagesColumnName()] = array_get(
                $file,
                'pages',
                0
            );
    ...
    protected function downloadFile($path)
    {
        ...
        $tmpPath = tempnam(
            config('mediatheque.fileable.tmp_path', sys_get_temp_dir()),
            'MEDIATHEQUE'
        );
    ...
    protected function getFileableDisk()
    {
        ...
        return $filesystem === 'cloud'
            ...
            : app('filesystem')->disk($filesystem);
    }
    ...
    protected function saveFile($path, $file)
    {
        ...
        if ($disk) {
            ...
            $disk->getDriver()->put($file['filename'], $resource, [
                'visibility' => 'public',
